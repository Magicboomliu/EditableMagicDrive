...
def points_inside_bboxes(lidar_points, bounding_boxes):
...
def _preprocess_bbox_and_the_lidar_points(bbox_mode, canvas_size, 
    ...
    # gt_instance_lidar_list = [points_inside_bboxes(example['points'].data) for example in examples]
    ...
    for idx in range(B):
        ...
        lidar_points_kf = gt_instance_lidar_list[idx] # LiDAR coordinate--->[N,5]
        ...
        bboxes.append([bboxes_pt[ind] for ind in index_list])
        classes.append([classes_kf[ind] for ind in index_list])
        lidares.append([lidar_points_kf[ind] for ind in index_lidar_list])
    ...
    for _b in range(B):
        _bboxes = bboxes[_b]
        _classes = classes[_b]        
        _lidars = lidares[_b] #(6,max_lidar,3)
...
def collate_fn_with_instance_lidar(
    examples: Tuple[dict, ...],
    template: str,
